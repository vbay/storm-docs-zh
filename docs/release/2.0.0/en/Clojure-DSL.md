...
`spout-spec` takes as arguments the spout implementation (an object that implements [IRichSpout](javadocs/org/apache/storm/topology/IRichSpout.md)) and optional keyword arguments. The only option that exists currently is the `:p` option, which specifies the parallelism for the spout. If you omit `:p`, the spout will execute as a single task.

#### bolt-spec

`bolt-spec` takes as arguments the input declaration for the bolt, the bolt implementation (an object that implements [IRichBolt](javadocs/org/apache/storm/topology/IRichBolt.md)), and optional keyword arguments.
...
See [Concepts](Concepts.md) for more info on stream groupings. Here's an example input declaration showcasing the various ways to declare inputs:
...
The syntax of output declarations is described in more detail in the `defbolt` section below. See [Using non JVM languages with Storm](Using-non-JVM-languages-with-Storm.md) for more details on how multilang works within Storm.
...
The stream id is a string, while the stream spec is either a vector of fields or a vector of fields wrapped by `direct-stream`. `direct stream` marks the stream as a direct stream (See [Concepts](Concepts.md) and [Direct groupings]() for more details on direct streams).
...
See [Guaranteeing message processing](Guaranteeing-message-processing.md) for more info on acking and anchoring.
...
[This blog post](http://www.pixelmachine.org/2011/12/17/Testing-Storm-Topologies.md) and its [follow-up](http://www.pixelmachine.org/2011/12/21/Testing-Storm-Topologies-Part-2.md) give a good overview of Storm's powerful built-in facilities for testing topologies in Clojure.
